...
function isCardFile(file: File): boolean {
  ...
  return lower.endsWith('.png') || lower.endsWith('.zip');
}
...
const Shell: React.FC = () => {
  ...
  const handleUploadSubmit = useCallback(async () => {
    ...
    if (!isCardFile(uploadedFile)) {
      window.dispatchEvent(
        new CustomEvent(CHAT_UPLOAD_EVENT, {
            ...
            {uploadedFile ? (
              ...
              <div className={styles.uploadDropZone} onClick={() => fileInputRef.current?.click()}>
                ...
                <p className={styles.uploadHint}>
                  Any document/image for chat context, or PNG/ZIP for card import
                </p>
              </div>
            )}
            <input
